...
#setwd 
setwd("/Users/don/Documents/R/Anomaly//")
...
#############
#For training set
legit_raw <- read.csv("Train_set.csv", header = TRUE) #tach0-tach78 are the variables, 96 observations
...
cft_raw <- read.csv("Input_Cft.csv", header = TRUE) #I won't really be using this much, but its good to see a set of ONLY counterfeits
...
########TESTING#################TESTING#################TESTING#########
#Read in Data
test_raw <- read.csv("Input_TestNew.csv", header = TRUE) #See comment line24.
##########what is test_raw?###########
...
#For test set
D2_Test_New <- mahalanobis(L_matrix, colMeans(Test_matrix), L_inv)
##########################
#SET: 1-D array of mahalanobis square distances for each observation.
...
write.table(D2_Test_New, file = "D2_Test_New.csv",row.names=FALSE, na="",col.names=FALSE, sep=",")
